...
### Rename to positive vs. negative feelings
```{r change-names}
...
### Binding all the variables to bootstrap/permute
```{r}
...
all$all <- factor(all$all, levels = c("no", "yes"))

ggplot(data = all, mapping = aes(x = pre_or_post, fill = all)) +
  geom_bar(position = "fill") +
  labs(title = "Proportion of Girls Who Want to Take More STEM Classes", subtitle = "Before and After Ignite", x = "Pre or Post Ignite", y = "Proportion", fill = " ")
```
...
ggplot(data = all, mapping = aes(x = pre_or_post, fill = all)) +
  geom_bar(position = "fill") +
  facet_grid(~ grade) +
  labs(title = "Proportion of Girls Who Want to Take More STEM Classes", subtitle = "By Grade, Before and After Ignite", x = "Pre or Post Ignite", y = "Proportion", fill = " ")
```
...
set.seed(2019)
permute_all <- all %>%
  specify(response = all, explanatory = pre_or_post, success = "yes") %>%
...
### More courses dataset for permute
```{r}
more_courses <- rbind(math_courses,
             science_courses,
             opportunity_engineering)

more_courses$all <- more_courses$all %>%
  str_replace("neutral", "no")
...
### Visualizing sample statistic
```{r}
more_courses$pre_or_post <- factor(more_courses$pre_or_post, levels = c("pre", "post"))
more_courses$all <- factor(more_courses$all, levels = c("no", "yes"))

ggplot(data = more_courses, mapping = aes(x = pre_or_post, fill = all)) +
  geom_bar(position = "fill") + labs(title= "Proportion of Girls", subtitle = "Who wanted to take more STEM courses Before and After Ignite", x="Pre or Post", y="Proportion", fill=" ")
```

```{r}
sampling_courses <- more_courses %>%
  group_by(pre_or_post) %>%
  count(all) %>%
  mutate(prop = (n / sum(n))) %>%
  ungroup(pre_or_post) %>%
...
```{r}
ggplot(data = permute_courses, 
       mapping = aes(x = stat)) +
  geom_histogram(binwidth=0.015) +
  labs(title = "Distribution of difference in proportion of girls", subtitle = "who wanted to take more STEM courses before and after Ignite", x = "Difference in Proportion", y = "Frequency") 
```

```{r}
permute_courses %>%
  filter(stat >= sampling_courses) %>%
  summarise((p_value = n()/1000) * 2)
```
...
```{r}
empower_contribute <- empower %>%
  select(pre_or_post, contribute_community)
names(empower_contribute) <- c("pre_or_post", "all")

empower_resolve <- empower %>%
  select(pre_or_post, resolve_problems_community)
names(empower_resolve) <- c("pre_or_post", "all")

empower_help <- empower %>%
  select(pre_or_post, external_help)
names(empower_help) <- c("pre_or_post", "all")

empower_leader <- empower %>%
  select(pre_or_post, leader_community)
names(empower_leader) <- c("pre_or_post", "all")

empower_contribute1 <- empower %>%
  select(pre_or_post, contribute_community.1)
names(empower_contribute1) <- c("pre_or_post", "all")

empower_bind <- rbind(empower_contribute, 
                 empower_resolve, 
                 empower_help, 
                 empower_leader, 
                 empower_contribute1)
...
empower_bind$all <- neutral(empower_bind$all)
  
empower_bind <- empower_bind %>%
  filter(!is.na(all))
```
...
```{r}
ggplot(data = boot_community, mapping = aes(x = stat)) +
  geom_histogram(binwidth = 0.01) +
  labs(title = "Difference in Proportion of Girls", subtitle = "Who Want to Do More for Their Community after Ignite",
...
boot_community %>%
  summarize(lower_bound = quantile(stat, 0.005),
            upper_bound = quantile(stat, 0.995))
```
I am 99% confident that the proportion of girls who want to do more for their community after the course is -0.043 to 0.13 greater than the proportion of girls wanted to do more for their community before the course. 
...
```{r}
empower_bind$pre_or_post <- factor(empower_bind$pre_or_post, levels = c("pre", "post"))
empower_bind$all <- factor(empower_bind$all, levels = c("no", "yes"))

ggplot(data = empower_bind, mapping = aes(x = pre_or_post, fill = all)) +
  geom_bar(position = "fill")
```
...
ggplot(data = permute, 
       mapping = aes(x = stat)) +
  geom_histogram() +
  labs(title = "Distribution of difference in mean weights", subtitle = "of babies born to smoking and non-smoking mothers", x = "Difference in Mean Weight", y = "Frequency") 
```
